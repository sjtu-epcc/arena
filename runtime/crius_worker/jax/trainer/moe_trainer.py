...
class MoETrainer:
    ...
    def __init__(self, trainer_cfgs, file_path, is_dp_only, is_pp_only, is_mp_only, is_manual_config_test, 
        ...
        self.expert_group_size = trainer_cfgs['expert_group_size']
        self.num_experts = trainer_cfgs['num_experts']
        # Load
    ...
    def _get_flax_model(self, model_cfgs):
        ...
        model_cfgs['intermediate_size'] = self.hidden_size * 8
        model_cfgs['num_attention_heads'] = self.num_heads
        model_cfgs['max_position_embeddings'] = self.seq_len
        model_cfgs['vocab_size'] = self.vocab_size
        model_cfgs['expert_group_size'] = self.expert_group_size
        model_cfgs['expert_number'] = self.num_experts
    ...
    def _get_init_state(self, batch):
        ...
        # Optmizer in Jax
        tx = optax.adafactor(learning_rate=1e-2, weight_decay_mask=weight_decay_mask)
        use_master_copy = (self.dtype == jnp.float16)
        # State
        state = TrainState2.create(apply_fn=self.model.apply, 
                                  params=params, 
                                  tx=tx, 
    ...
    def _get_moe_stat(self, partial_cfgs, latencies, num_devices):
        """
        Compute statistics of MoE.
        """
        ...
        num_experts = partial_cfgs['num_experts']
        expert_group_size = partial_cfgs['expert_group_size']
        ...
        parameter_count = compute_moe_parameter_count(num_layers, hidden_size,
                                                    vocab_size, num_experts, mlp_factor=8)
    ...
    def train(self, dump_debug_file_path=None, job_id=None, try_idx=None):
        ...
        #         self.batch_size * self.seq_len // self.num_micro_batches // 1 // rang_factor)
        #     if expected_expert_group_size != self.expert_group_size:
        #         print("- Expected expert group size should be {}, "
        #             "but got {}. Will reset it".format(expected_expert_group_size,
        #                                                 self.expert_group_size))
        #         self.expert_group_size = expected_expert_group_size
        ...
        # State
        state = self._get_init_state(batch=batch)
     ...
     executable) = compile_and_benchmark_pipeshard_training_executable(
        ...
        # }
        # tflops, parameter_count = self._get_moe_stat(partial_cfgs=partial_cfgs, latencies=latencies, num_devices=(num_nodes * num_devices_per_node))
