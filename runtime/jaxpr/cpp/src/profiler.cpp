...
#include "../external/habitat-cu116/cpp/src/cuda/habitat_cupti.h"
...
std::vector<KernelInstance> measure(
...
void define_properties(pybind11::module& m) {
    ...
    .def_property_readonly("num_blocks", [](const KernelInstance& self) { 
        return self.metadata().numBlocks(); 
    })
...
std::vector<KernelInstance> profile(std::function<void()> runnable, const std::string& metric) {
  std::vector<KernelInstance> kernels(measure(runnable));
