...
"""
For generating runtime trace for the heterogeneous cluster with H100 and L20 GPUs.
...
parser.add_argument("--visualize_raw_trace", default=False, action="store_true", 
                    help="Analyze and visualize raw trace.")
...
prob_list_param_num = [20, 40, 60, 80, 100, 120]
# Probability proportion for gpu num
prob_list_gpu_num = [10, 20, 40, 100, 200]
# Probability proportion for batch size
prob_list_batch_size = [50, 60, 70]


supported_gpu_type = ["h100", ]
""" Supported GPU types in this trace. """

ITER_NUM_SCALE_FACTOR = 20
""" Scale factor for iteration num in runtime trace. """

RUNTIME_MAX_ITER_NUM = 3000
""" Maximal iteration num for each job in runtime trace. """
...
def load_and_regen_trace(file_path: str = "./traces/runtime_trace_raw.csv", 
                         target_file_path: str = "./traces/dummy_runtime_trace_revision.csv"): 
    """ 
    ...
    for _i, _rec in enumerate(data_list):
        ...
        while not is_feasible:
            ...
            # GPU type
            _gpu_type = supported_gpu_type[0]
...
def analyze_raw_trace(trace_pth: str):
    ...
    file_path = f"./figures/job_distribution_runtime_revision.pdf" 
    plt.savefig(file_path, bbox_inches='tight')


if __name__ == "__main__":
    if args.visualize_raw_trace:
        analyze_raw_trace(args.raw_trace_path)
    else:
        target_pth = "./traces/runtime_trace_revision_homo.csv" if not args.dummy_test \
                        else "./traces/dummy_runtime_trace_revision_homo.csv"
        raw_trace_path = args.raw_trace_path if args.raw_trace_path != "none" \
                            else "./traces/runtime_trace_raw.csv"
        if args.dummy_test:
            print("Generate dummy runtime trace for test ...")
            raw_trace_path = "./traces/dummy_runtime_trace_raw.csv"
        ...
        load_and_regen_trace(raw_trace_path, target_pth)
